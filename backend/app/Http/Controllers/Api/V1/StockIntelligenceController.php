...
{
    /**
     * Curva ABC — Classificação de produtos por valor movimentado (Pareto 80/20).
     */
    ...
    /**
     * Giro de Estoque — Mede velocidade de renovação do estoque.
     */
    public function turnover(Request $request): JsonResponse
    {
        ...
        $result = $items->map(function ($item) use ($months) {
            ...
            // Giro = Saídas no período / Estoque médio (simplificado: estoque atual)
            $turnoverRate = $stockQty > 0 ? round($exits / $stockQty, 2) : ($exits > 0 ? 999 : 0);

            // Cobertura em dias (quanto tempo o estoque atual duraria)
    ...
    /**
     * Custo Médio Ponderado — Calcula custo médio por produto.
     */
    ...
    /**
     * Pontos de Reposição — Produtos abaixo do ponto mínimo.
     */
    public function reorderPoints(Request $request): JsonResponse
    {
        ...
        $result = $items->map(function ($item) use ($months) {
            ...
            $suggestedQty = max(0, round($stockMin * 2 - $stockQty, 2)); // Repor até 2x o mínimo
        ...
        // Filtrar só os que precisam de atenção
    ...
    /**
     * Reservas de estoque ativas (via movimentações do tipo 'reserve').
     */
