...
export const SelectNative = forwardRef<HTMLSelectElement, SelectProps>(
    ({ className, label, error, children, ...props }, ref) => (
        <div className="space-y-1.5">
            {label && <label className="block text-[13px] font-medium text-surface-700 dark:text-surface-300">{label}</label>}
            <select
                ref={ref}
                className={cn(
                    'block w-full rounded-md border bg-surface-50 dark:bg-surface-800/50 px-3 py-2 text-sm text-surface-900 dark:text-surface-100',
