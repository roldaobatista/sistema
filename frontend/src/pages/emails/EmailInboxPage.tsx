...
import {
    ...
    Reply, Forward, Paperclip, CheckCheck, Clock, Sparkles, Tag,
    AlertCircle, ArrowRight, Plus, Eye, EyeOff, ChevronDown, Loader2,
...
export default function EmailInboxPage() {
  ...
  const { data: items, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => api.get('/email-inbox').then(r => r.data?.data ?? r.data ?? []),
  })

  // MVP: Delete mutation
  const queryClient = useQueryClient()
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/email-inbox/${id}`),
    onSuccess: () => { toast.success('Removido com sucesso'); queryClient.invalidateQueries({ queryKey: ['email-inbox'] }) },
                        ...
                        emails.map((e: EmailItem) => (
                            ...
                            >
                                ...
                                <div className="text-right shrink-0 space-y-1">
                                    <p className="text-xs text-muted-foreground">{formatEmailDate(e.received_at)}</p>
