...
import { Loader2, Map as MapIcon, RotateCw, Wand2 } from 'lucide-react'
...
export function TechnicianMap({ items, technicianId }: TechnicianMapProps) {
  ...
  const { data: items, isLoading, isError, refetch } = useQuery({
    queryKey: ['technician-map'],
    queryFn: () => api.get('/technician-map').then(r => r.data?.data ?? r.data ?? []),
  })

  // MVP: Delete mutation
  const queryClient = useQueryClient()
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/technician-map/${id}`),
    onSuccess: () => { toast.success('Removido com sucesso'); queryClient.invalidateQueries({ queryKey: ['technician-map'] }) },
  ...
  const hasPermission = (p: string) => user?.all_permissions?.includes(p) ?? false
    ...
    const handleOptimize = async () => {
        ...
        try {
            ...
            const optimizedOrder = response.data // array of WorkOrders in order
