...
export default function TechProfilePage() {
  ...
  const { data: items, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => api.get('/tech-profile').then(r => r.data?.data ?? r.data ?? []),
  })

  // MVP: Delete mutation
  const queryClient = useQueryClient()
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/tech-profile/${id}`),
    onSuccess: () => { toast.success('Removido com sucesso'); queryClient.invalidateQueries({ queryKey: ['tech-profile'] }) },
    ...
    const clearLocalData = async () => {
        ...
        try {
            ...
            const storeNames: string[] = [
                'work-orders', 'equipment', 'checklists',
                'checklist-responses', 'expenses', 'photos',
                'signatures', 'sync-metadata',
            ]
            const tx = db.transaction(storeNames as any, 'readwrite')
            await Promise.all(storeNames.map((store) => tx.objectStore(store as any).clear()))
            await tx.done
        } catch {
            // Ignore
        } finally {
