...
import {
    ...
    Shield, ShieldCheck, Maximize2, Minimize2,
...
export default function TechSettingsPage() {
  ...
  const { data: items, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => api.get('/tech-settings').then(r => r.data?.data ?? r.data ?? []),
  })

  // MVP: Delete mutation
  const queryClient = useQueryClient()
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/tech-settings/${id}`),
    onSuccess: () => { toast.success('Removido com sucesso'); queryClient.invalidateQueries({ queryKey: ['tech-settings'] }) },
