...
import { ArrowLeft, Mic, MicOff, Save, Trash2, FileText, Copy, Check , Loader2 } from 'lucide-react'
...
export default function TechVoiceReportPage() {
  ...
  const { data: items, isLoading, isError, refetch } = useQuery({
    queryKey: ['tech-voice-report'],
    queryFn: () => api.get('/tech-voice-report').then(r => r.data?.data ?? r.data ?? []),
  })

  // MVP: Delete mutation
  const queryClient = useQueryClient()
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/tech-voice-report/${id}`),
    onSuccess: () => { toast.success('Removido com sucesso'); queryClient.invalidateQueries({ queryKey: ['tech-voice-report'] }) },
  ...
  const { hasPermission } = useAuthStore()
    ...
    const handleSave = () => {
        ...
        const key = `voice-report-os-${id || 'draft'}`
        const existing = JSON.parse(localStorage.getItem('voice-reports') || '[]')
        existing.push({
            id: Date.now(),
