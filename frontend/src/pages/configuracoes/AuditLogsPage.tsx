...
export function AuditLogsPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/audit-logs/${id}`),
    onSuccess: () => { toast.success('Removido com sucesso'); queryClient.invalidateQueries({ queryKey: ['audit-logs'] }) },
  ...
  const { hasPermission } = useAuthStore()
    ...
    const renderDiff = (oldVal: any, newVal: any) => {
        if (!oldVal && !newVal) return <p className="text-[13px] text-surface-500 italic">Sem alterações registradas.</p>
        ...
        if (changes.length === 0) return <p className="text-[13px] text-surface-500 italic">Nenhuma mudança identificada.</p>

        return (
            <div className="space-y-3">
                {changes.map(key => (
                    <div key={key} className="grid grid-cols-2 gap-4 rounded-lg border border-subtle p-2.5 text-sm">
                        <div>
    ...
    return (
        <div className="space-y-5 animate-fade-in">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-lg font-semibold text-surface-900 tracking-tight">Logs de Auditoria</h1>
                    <p className="mt-0.5 text-[13px] text-surface-500">Rastreabilidade e histórico de alterações</p>
                </div>
            </div>

            {/* Filters */}
            <div className="flex flex-wrap gap-3 rounded-xl border border-default bg-surface-0 p-4 shadow-sm">
                <div className="relative flex-1 min-w-[200px]">
                    <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-surface-400" />
                    <input
                        value={search} onChange={e => setSearch(e.target.value)}
                        placeholder="Buscar em descrições e valores..."
                        className="w-full rounded-lg border border-default bg-surface-50 py-2 pl-10 pr-4 text-sm focus:border-brand-500 focus:outline-none"
                    />
                </div>
                <select
            ...
            <div className="overflow-hidden rounded-xl border border-default bg-surface-0 shadow-card">
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-subtle bg-surface-50">
                            ...
                            <th className="px-3.5 py-2.5 text-left text-xs font-semibold uppercase text-surface-600">Recurso</th>
                            <th className="px-3.5 py-2.5 text-left text-xs font-semibold uppercase text-surface-600">Descrição</th>
                            <th className="px-3.5 py-2.5 text-right text-xs font-semibold uppercase text-surface-600">Detalhes</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-subtle">
                        {isLoading ? (
                            ...
                            <tr><td colSpan={6} className="px-4 py-12 text-center text-[13px] text-surface-500">Nenhum registro encontrado</td></tr>
                        ) : logs.map((log: any) => (
                            <tr key={log.id} className="hover:bg-surface-50 transition-colors duration-100">
                                <td className="px-4 py-3 text-[13px] text-surface-500 whitespace-nowrap">
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[13px] font-medium text-surface-900">{log.user?.name ?? 'Sistema'}</span>
                                    </div>
                                </td>
                                <td className="px-4 py-3">
                ...
                <div className="flex items-center justify-between border-t border-subtle px-4 py-3">
                    ...
                    <span className="text-[13px] text-surface-600">Página {meta.current_page} de {meta.last_page}</span>
